...
<template>
  <div class="function-tabs">
    <div>
      <nav class="nav nav-pills va-row">
        <div
          ...
          :class="{active: name === activeFunctionDetailTab}"
          v-for="name in names"
          :key="name"
        >
    ...
    <vuestic-simple-select
      class="simple-select"
      v-show="false"
      name="function-tab-select"
      v-bind:options="names" v-model="activeFunctionDetailTab"></vuestic-simple-select>
...
export default {
  ...
  computed: {
    ...
    underscoreClass () {
      return 'underscore-' + this.names.length + '-' + this.names.indexOf(this.activeFunctionDetailTab)
    },
  },
  methods: {
    setActive (name) {
      this.$store.dispatch('setActiveFunctionDetailTab', name)
    },
  }
}
</script>
