<template>
    <div class="function-instances">
        ...
        <div class="va-row">
            <div class="flex md12">
                <vuestic-widget v-if="functionsData.status[$route.params.id]">
                    <div class="va-row">
                        ...
                        <div class="flex md12">
                            <div class="va-row">
                                <div v-for="statKey in narrowCopyKeys" :key="statKey" class="flex md6">
                                    <div  class="form-group with-icon-right">
                                        ...
                                        <input :value="getHumanReadable(statKey, functionsData.status[$route.params.id][statKey])" :id="statKey" :name="statKey"
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped first-td-padding table-header-active">
                                    ...
                                    <tbody>
                                            <tr v-for="(inputInfo, index) in functionsData.status[$route.params.id].instances" :key="inputInfo.instanceId">

                                               <td>
                                                    <button v-show="functionsData.status[$route.params.id].instances[index].status.running" class="btn btn-primary btn-micro btn-bottom-margin" @click="confirmStopInstance(inputInfo.instanceId)">Stop</button>
                                                    <button v-show="!functionsData.status[$route.params.id].instances[index].status.running" class="btn btn-primary btn-micro btn-bottom-margin" @click="confirmStartInstance(inputInfo.instanceId)">Start</button>
                                                    <button v-show="functionsData.status[$route.params.id].instances[index].status.running" class="btn btn-primary btn-micro btn-bottom-margin" @click="confirmRestartInstance(inputInfo.instanceId)">Restart</button>
                                               </td>
...
export default {
  ...
  methods: {
    ...
    async stopInstance () {
      ...
      const functionPath = this.functionsData.data[this.$route.params.id].path
      const cluster = this.functionsData.data[this.$route.params.id].cluster
      // Wait for the API call to return, then update the topic so the user gets feedback
      try {
        await ApiService.stopFunctionInstance(cluster, functionPath, instanceId)
        // Force an update to the topic immediately
        this.$store.dispatch('updateFunction', this.functionsData.data[this.$route.params.id])
    ...
    async startInstance () {
      ...
      // Wait for the API call to return, then update the topic so the user gets feedback
      try {
        await ApiService.startFunctionInstance(cluster, functionPath, instanceId)
    ...
    async restartInstance () {
      ...
      // Wait for the API call to return, then update the topic so the user gets feedback
      try {
        await ApiService.restartFunctionInstance(cluster, functionPath, instanceId)
        // Not going to force an update because the state is temporarily messed up while starting
        // this.$store.dispatch('updateFunction', this.functionsData.data[this.$route.params.id])
    ...
    async stopAllInstances () {
      ...
      // Wait for the API call to return, then update the topic so the user gets feedback
      try {
        await ApiService.stopAllFunctionInstances(cluster, functionPath)
        // Force an update to the topic immediately
        this.$store.dispatch('updateFunction', this.functionsData.data[this.$route.params.id])
    ...
    async startAllInstances () {
      ...
      // Wait for the API call to return, then update the topic so the user gets feedback
      try {
        const response = await ApiService.startAllFunctionInstances(cluster, functionPath)
        // Force an update to the topic immediately
        this.$store.dispatch('updateFunction', this.functionsData.data[this.$route.params.id])
    ...
    async restartAllInstances () {
      ...
      // Wait for the API call to return, then update the topic so the user gets feedback
      try {
        const response = await ApiService.restartAllFunctionInstances(cluster, functionPath)
        // Force an update to the topic immediately
        this.$store.dispatch('updateFunction', this.functionsData.data[this.$route.params.id])
