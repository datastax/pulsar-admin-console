<template>
  <div class="sink-tabs">
    <div>
      <nav class="nav nav-pills va-row">
        <div
          ...
          :class="{active: name === activeSinkDetailTab}"
          v-for="name in names"
          :key="name"
        >
    ...
    <vuestic-simple-select
      class="simple-select"
      v-show="false"
      name="sink-tab-select"
      v-bind:options="names" v-model="activeSinkDetailTab"></vuestic-simple-select>
...
export default {
  ...
  computed: {
    ...
    underscoreClass () {
      return 'underscore-' + this.names.length + '-' + this.names.indexOf(this.activeSinkDetailTab)
    },
  },
  methods: {
    setActive (name) {
      this.$store.dispatch('setActiveSinkDetailTab', name)
    },
  }
}
</script>
