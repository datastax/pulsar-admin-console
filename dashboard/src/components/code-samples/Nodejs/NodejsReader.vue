...
<template>
    <div class="java-reader">
        <h5>Simple Reader</h5>
        <p>This sample creates a reader on a topic and reads the earliest or latest message from the topic up to 1000 messages.</p>
...
(async () => {
  ...
  const reader = await client.createReader({
    topic: topicName,
    startMessageId: Pulsar.MessageId.${this.currentPosition.id}(),
  });
