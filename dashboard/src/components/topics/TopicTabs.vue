<template>
  <div class="vuestic-tabs">
    <div>
      <nav class="nav nav-pills va-row">
        <div
          ...
          :class="{active: name === activeTopicDetailTab}"
          v-for="name in names"
          :key="name"
        >
    ...
    <vuestic-simple-select
      class="simple-select"
      v-show="false"
      v-bind:options="names" v-model="activeTopicDetailTab"></vuestic-simple-select>
...
export default {
  ...
  computed: {
    ...
    underscoreClass () {
      return 'underscore-' + this.names.length + '-' + this.names.indexOf(this.activeTopicDetailTab)
    },
  },
  methods: {
    setActive (name) {
      this.$store.dispatch('setActiveTopicDetailTab', name)
    },
  }
}
</script>
