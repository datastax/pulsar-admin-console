...
<template>
  <div class="broker-tabs">
    <div>
      <nav class="nav nav-pills va-row">
        <div
          ...
          :class="{active: name === activeBrokerDetailTab}"
          v-for="name in names"
          :key="name"
        >
    ...
    <vuestic-simple-select
      class="simple-select"
      v-show="false"
      name="broker-tab-select"
      v-bind:options="names" v-model="activeBrokerDetailTab"></vuestic-simple-select>
...
export default {
  ...
  computed: {
    ...
    underscoreClass () {
      return 'underscore-' + this.names.length + '-' + this.names.indexOf(this.activeBrokerDetailTab)
    },
  },
  methods: {
    setActive (name) {
      this.$store.dispatch('setActiveBrokerDetailTab', name)
    },
  }
}
</script>
