...
export default {
  ...
  computed: {
    ...
    instanceSelect () {
      ...
      if (this.sourcesData.status[this.$route.params.id]) {
        this.sourcesData.status[this.$route.params.id].instances.forEach(key => {
          options.push({
  ...
  methods: {
    ...
    pollForLogs () {
      if (this.logsPolling && this.activeSourceDetailTab === this.$t('sourceDetail.tabs.logs')) {
    ...
    async getLogsBack () {
      ...
      try {
        const response = await ApiService.getFunctionLogs(cluster, sourcePath, this.backwardPosition, 0, this.currentInstance.id)
        // console.log(response.data)
        if (this.logData.includes('ERROR')) {
          this.logData = ''
        }
        ...
        this.errorText = `ERROR: ${reason} (${statusCode})`
        this.logData = this.errorText
    ...
    async getLogsForward () {
      ...
      try {
        const response = await ApiService.getFunctionLogs(cluster, sourcePath, 0, this.forwardPosition, this.currentInstance.id)
        if (this.logData.includes('ERROR')) {
          this.logData = ''
        }
